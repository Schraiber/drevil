...
for (i in 1:40) {
	...
	cur_opt = Ne_opt <- optimize_with_MOM_theta(SFS_tibble,num_replace,AN,0,Ne,R,T,nc=20,K=K,perturb_start=1)
...
full_opt = optimize_full_likelihood_const(SFS_tibble, num_replace, AN, 0, new_Ne, R, T, nc = 20,rel_reduce = sqrt(.Machine$double.eps),max_iter=5,perturb_start = 0,K = K)
...
print("Writing theta")
write_tsv(mutate(full_opt$theta_opt,AN=AN),paste0(outfile,".theta"))
